...
export default function ProductDetailsScreen() {
  ...
  const handleUploadImage = async () => {
    try {
      // 1. Request permissions first to prevent silent failures
      if (Platform.OS !== 'web') {
      ...
      const result = await ImagePicker.launchImageLibraryAsync({
        ...
        allowsEditing: false, // Set to false to allow full uncropped image upload
        quality: 0.8,
      });
      ...
      if (!result.canceled && result.assets?.length > 0) {
        ...
        // 3. Format FormData carefully for the backend
        const formData = new FormData();
        const filename = asset.fileName || asset.uri.split('/').pop() || `product_${Date.now()}.jpg`;
      ...
      Alert.alert('Upload Failed', err.response?.data?.message || 'Check your connection and try again.');
    } finally {
      setIsUploading(false);
    }
  };

  const handleOpenAction = async (action: 'adjust' | 'transfer') => {
    ...
    try {
      ...
      if (bList.length > 0) {
        setAdjustBranchId(bList[0]._id);
        setTransferFromBranchId(bList[0]._id);
      }
  ...
  const submitAdjustment = async () => {
    ...
    if (!adjustBranchId) return Alert.alert('Validation Error', 'Target branch is required.');
    if (isNaN(qty) || qty <= 0) return Alert.alert('Validation Error', 'Quantity must be greater than 0.');
    if (!adjustReason || adjustReason.length < 3) return Alert.alert('Validation Error', 'Reason is required (min 3 chars).');
  ...
  const submitTransfer = async () => {
    ...
    if (!transferFromBranchId || !transferToBranchId) return Alert.alert('Validation Error', 'Source and Destination branches are required.');
    if (transferFromBranchId === transferToBranchId) return Alert.alert('Conflict', 'Cannot transfer to the same branch.');
    if (isNaN(qty) || qty <= 0) return Alert.alert('Validation Error', 'Quantity must be greater than 0.');

    setIsProcessing(true);
    try {
      await ProductService.transferProductStock(product._id, {
        fromBranchId: transferFromBranchId,
  ...
  const handleDelete = () => {
    Alert.alert('Confirm Deletion', `Delete ${product?.name}?`, [
  ...
  const CustomDropdown = ({ label, value, options, onSelect, placeholder }: any) => {
    const [expanded, setExpanded] = useState(false);
    const selected = options.find((o: any) => o._id === value);
    return (
      <View style={styles.formField}>
        <ThemedText style={styles.formLabel}>{label}</ThemedText>
        <TouchableOpacity style={styles.dropdownSim} onPress={() => setExpanded(!expanded)}>
          <ThemedText style={{ color: selected ? theme.textPrimary : theme.textTertiary, fontFamily: theme.fonts.body }}>
            {selected ? selected.name : placeholder}
          </ThemedText>
          <Ionicons name={expanded ? "chevron-up" : "chevron-down"} size={20} color={theme.textTertiary} />
        </TouchableOpacity>
        {expanded && (
          <View style={styles.dropdownList}>
            {options.map((o: any) => (
              <TouchableOpacity
                key={o._id}
                style={[styles.dropdownItem, value === o._id && { backgroundColor: `${theme.accentPrimary}05` }]}
                onPress={() => { onSelect(o._id); setExpanded(false); }}
              >
                <ThemedText style={{ color: value === o._id ? theme.accentPrimary : theme.textPrimary, fontWeight: value === o._id ? 'bold' : 'normal' }}>{o.name}</ThemedText>
              </TouchableOpacity>
            ))}
          </View>
        )}
  ...
  const calculateMargin = (p: any) => (((p?.sellingPrice - p?.purchasePrice) / p?.sellingPrice) * 100).toFixed(1) || '0.0';

  if (isLoading && !isRefreshing) {
    return (
      <ThemedView style={styles.center}><ActivityIndicator size="large" color={theme.accentPrimary} /></ThemedView>
    );
  }
  ...
  if (errorMsg || !product) {
    return (
      <ThemedView style={styles.center}>
        ...
        <TouchableOpacity style={styles.backBtn} onPress={() => router.navigate('/(tabs)/product' as any)}>
  ...
  return (
    <ThemedView style={styles.container}>
      <SafeAreaView style={styles.safeArea} edges={['bottom', 'left', 'right']}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={() => loadProductData(true)} tintColor={theme.accentPrimary} />}>
          ...
          <View style={styles.identitySection}>
            <View style={styles.titleRow}>
              ...
              <View style={[styles.badge, { backgroundColor: product.isActive ? `${theme.success}15` : `${theme.error}15` }]}>
                <ThemedText style={[styles.badgeText, { color: product.isActive ? theme.success : theme.error }]}>{product.isActive ? 'ACTIVE' : 'INACTIVE'}</ThemedText>
              </View>
            </View>
          ...
          <View style={styles.metricsGrid}>
            <View style={styles.metricCard}><View style={styles.metricHeader}><ThemedText style={styles.metricLabel}>Selling Price</ThemedText></View><ThemedText style={[styles.metricValue, { color: theme.success }]}>{formatCurrency(product.sellingPrice)}</ThemedText></View>
            <View style={styles.metricCard}><View style={styles.metricHeader}><ThemedText style={styles.metricLabel}>Purchase Cost</ThemedText></View><ThemedText style={styles.metricValue}>{formatCurrency(product.purchasePrice)}</ThemedText></View>
            <View style={styles.metricCard}><View style={styles.metricHeader}><ThemedText style={styles.metricLabel}>Total Stock</ThemedText></View><ThemedText style={styles.metricValue}>{product.totalStock} {product.unitId?.name || 'pcs'}</ThemedText></View>
            <View style={styles.metricCard}><View style={styles.metricHeader}><ThemedText style={styles.metricLabel}>Est. Margin</ThemedText></View><ThemedText style={styles.metricValue}>{calculateMargin(product)}%</ThemedText></View>
          </View>
          ...
          <View style={styles.card}>
            ...
            {product.inventory?.length > 0 ? (
              product.inventory.map((inv: any, idx: number) => {
                ...
                const isLow = qty <= (inv.reorderLevel || 0);
                return (
                  <View key={idx} style={styles.inventoryRow}>
                    <View style={{ flex: 1 }}>
                      <ThemedText style={styles.branchName}>{inv.branchId?.name || 'Head Office'}</ThemedText>
                      <ThemedText style={styles.reorderText}>Reorder Level: {inv.reorderLevel || 0}</ThemedText>
                    </View>
      ...
      <Modal visible={showActionMenu} transparent animationType="fade">
        <TouchableOpacity style={styles.modalOverlay} activeOpacity={1} onPress={() => setShowActionMenu(false)}>
          <View style={[styles.bottomSheet, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => handleOpenAction('adjust')}>
            ...
            <TouchableOpacity style={styles.actionItem} onPress={() => handleOpenAction('transfer')}>
          ...
          <View style={[styles.formBottomSheet, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            ...
            <ScrollView bounces={false} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={styles.formBody}>
                {/* Segment for Add/Subtract */}
                <View style={styles.segmentContainer}>
                  <TouchableOpacity style={[styles.segmentBtn, adjustType === 'add' && { backgroundColor: theme.success }]} onPress={() => setAdjustType('add')}>
                    <ThemedText style={[styles.segmentText, adjustType === 'add' && { color: theme.bgSecondary }]}>Add (+)</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.segmentBtn, adjustType === 'subtract' && { backgroundColor: theme.error }]} onPress={() => setAdjustType('subtract')}>
                    <ThemedText style={[styles.segmentText, adjustType === 'subtract' && { color: theme.bgSecondary }]}>Remove (-)</ThemedText>
                  </TouchableOpacity>
                </View>

                <CustomDropdown label="Target Branch" value={adjustBranchId} options={branches} onSelect={setAdjustBranchId} placeholder="Select Branch" />

                <View style={styles.formField}>
                  <ThemedText style={styles.formLabel}>Adjustment Quantity</ThemedText>
                  <TextInput style={styles.modalInput} keyboardType="numeric" value={adjustQty} onChangeText={setAdjustQty} placeholder="e.g. 10" placeholderTextColor={theme.textTertiary} />
                </View>
                <View style={styles.formField}>
                  <ThemedText style={styles.formLabel}>Reason / Audit Note</ThemedText>
                  <TextInput style={styles.modalInput} value={adjustReason} onChangeText={setAdjustReason} placeholder="Damaged, found extra..." placeholderTextColor={theme.textTertiary} />
                </View>
                <TouchableOpacity style={[styles.submitBtn, { backgroundColor: adjustType === 'add' ? theme.success : theme.error }]} onPress={submitAdjustment} disabled={isProcessing}>
          ...
          <View style={[styles.formBottomSheet, { paddingBottom: Math.max(insets.bottom, 20) }]}>
            ...
            <ScrollView bounces={false} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
              <View style={styles.formBody}>
                ...
                {/* Transfer Visual Graphic */}
                <View style={styles.transferVisual}>
                  <View style={styles.nodeBox}>
                    <Ionicons name="business" size={20} color={transferFromBranchId ? theme.accentPrimary : theme.textTertiary} />
                    <ThemedText style={styles.nodeText}>Source</ThemedText>
                  </View>
                  <View style={styles.flowLine}>
                    {transferFromBranchId && transferToBranchId && <View style={styles.movingDot} />}
                  </View>
                  <View style={styles.nodeBox}>
                    <Ionicons name="location" size={20} color={transferToBranchId ? theme.accentPrimary : theme.textTertiary} />
                    <ThemedText style={styles.nodeText}>Destination</ThemedText>
                  </View>
                </View>

                <CustomDropdown label="From Branch" value={transferFromBranchId} options={branches} onSelect={setTransferFromBranchId} placeholder="Select Source Branch" />
                <CustomDropdown label="To Branch" value={transferToBranchId} options={branches} onSelect={setTransferToBranchId} placeholder="Select Destination Branch" />

                <View style={styles.formField}>
                  <ThemedText style={styles.formLabel}>Transfer Quantity</ThemedText>
                  <TextInput style={styles.modalInput} keyboardType="numeric" value={transferQty} onChangeText={setTransferQty} placeholder="Quantity to move" placeholderTextColor={theme.textTertiary} />
                </View>
                <View style={styles.formField}>
                  <ThemedText style={styles.formLabel}>Reference / Note (Optional)</ThemedText>
                  <TextInput style={styles.modalInput} value={transferDesc} onChangeText={setTransferDesc} placeholder="Transfer details..." placeholderTextColor={theme.textTertiary} />
                </View>
      ...
      <Modal visible={!!selectedImage} transparent animationType="fade">
        <View style={styles.fullScreenViewer}>
          <TouchableOpacity style={styles.viewerCloseBtn} onPress={() => setSelectedImage(null)}>
...
const createStyles = (theme: ThemeColors, insets: any) => StyleSheet.create({
  ...
  metricLabel: { fontFamily: theme.fonts.body, fontSize: Typography.size.xs, fontWeight: Typography.weight.bold, color: theme.textSecondary, textTransform: 'uppercase' },
  ...
  bottomSheet: { backgroundColor: theme.bgSecondary, borderTopLeftRadius: UI.borderRadius.xl, borderTopRightRadius: UI.borderRadius.xl },
  formBottomSheet: { backgroundColor: theme.bgPrimary, borderTopLeftRadius: UI.borderRadius.xl, borderTopRightRadius: UI.borderRadius.xl, maxHeight: '90%' },
  ...
  actionItem: { flexDirection: 'row', alignItems: 'center', gap: Spacing.lg, padding: Spacing.xl, backgroundColor: theme.bgPrimary, borderBottomWidth: UI.borderWidth.thin, borderBottomColor: theme.borderPrimary },
  ...
  formLabel: { fontFamily: theme.fonts.body, fontSize: Typography.size.sm, fontWeight: Typography.weight.bold, color: theme.textSecondary, marginBottom: Spacing.xs, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  submitBtn: { height: 56, backgroundColor: theme.accentPrimary, borderRadius: UI.borderRadius.lg, alignItems: 'center', justifyContent: 'center', marginTop: Spacing.sm, ...getElevation(2, theme) },
  ...
  // SEGMENTED CONTROL (Adjust Modal)
  segmentContainer: { flexDirection: 'row', backgroundColor: theme.bgSecondary, padding: 4, borderRadius: UI.borderRadius.md, marginBottom: Spacing.xl, borderWidth: UI.borderWidth.thin, borderColor: theme.borderPrimary },
  segmentBtn: { flex: 1, paddingVertical: Spacing.md, alignItems: 'center', borderRadius: UI.borderRadius.sm },
  ...
  // TRANSFER VISUAL GRAPHIC (Transfer Modal)
  transferVisual: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: Spacing.xl, paddingHorizontal: Spacing.lg },
  nodeBox: { alignItems: 'center', gap: 8, width: 80 },
  nodeText: { fontFamily: theme.fonts.body, fontSize: Typography.size.xs, fontWeight: Typography.weight.bold, color: theme.textSecondary },
  flowLine: { flex: 1, height: 2, backgroundColor: theme.borderPrimary, marginHorizontal: Spacing.md, overflow: 'hidden' },
  movingDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: theme.accentPrimary, alignSelf: 'center', marginTop: -3 },

  // INLINE DROPDOWN SIMULATOR
  dropdownSim: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', height: 56, backgroundColor: theme.bgSecondary, borderRadius: UI.borderRadius.md, paddingHorizontal: Spacing.xl, borderWidth: UI.borderWidth.thin, borderColor: theme.borderPrimary },
  dropdownList: { backgroundColor: theme.bgPrimary, borderRadius: UI.borderRadius.md, borderWidth: UI.borderWidth.thin, borderColor: theme.borderPrimary, marginTop: 4, maxHeight: 150, overflow: 'hidden' },
  dropdownItem: { padding: Spacing.md, borderBottomWidth: UI.borderWidth.thin, borderBottomColor: theme.borderPrimary },
